...
export default function ProjectsBento() {
  return <section className="section projects-bento-section" id="work"><div className="section-head"><div><p className="eyebrow">04 / AI architecture showcase</p><h2>Systems that<br /><em>understand.</em></h2></div><p>Deep AI fundamentals made tangible through interpretability, tokenization, retrieval, and production diagnostics.</p></div><div className="projects-bento-grid">{projects.map((project, index) => <motion.article className="projects-bento-card bg-white border-zinc-200 dark:bg-[#18181b] dark:border-zinc-800" key={project.title} whileHover={{ y: -6 }} transition={{ duration: .25 }}><span className="mono project-index">0{index + 1}</span><span className="project-arrow" aria-hidden="true">↗</span><p className="mono project-category">{project.category}</p><h3>{project.title}</h3><p className="project-description">{project.description}</p><p className="project-metrics">{project.metrics}</p><div className="project-tech">{project.tech.map((tag) => <span key={tag}>{tag}</span>)}</div></motion.article>)}</div></section>;
}
